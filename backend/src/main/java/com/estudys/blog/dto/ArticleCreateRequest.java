...
public class ArticleCreateRequest {
    ...
    @Schema(description = "文章标题", example = "Spring Boot 实战总结")
    private String title;

    @NotBlank(message = "content is required")
    @Schema(description = "文章内容(Markdown)", example = "# 标题\\n这是正文")
    private String content;

    @Size(max = 500, message = "summary length must <= 500")
    @Schema(description = "文章摘要", example = "这是一篇 Spring Boot 实战总结")
    private String summary;

    @Schema(description = "分类ID", example = "1001")
    private Long categoryId;
    @Schema(description = "封面图URL", example = "https://cdn.example.com/cover.png")
